        ...
        void FixedUpdate() {
            ...
            if (Physics.Raycast(transform.position, fwd, out hit)) {
				...
				/*if(lastTargetName!=hit.transform.name){
					Debug.Log("Tag:"+hit.transform.tag+" Name:"+hit.transform.name+" last target:"+lastTargetName);
					lastTargetName = hit.transform.name;
				}*/
                if (hit.transform.tag == "VRGazeInteractable") {	
                    // Check if we have already gazed over the object.
					//Debug.Log("In VRGazeInteractable:"+lastTargetName+" transformName:"+hit.transform.name); 
                    if (lastTargetName == hit.transform.name) {
                        return;
                    }
